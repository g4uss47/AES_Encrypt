    ...
    class Program
    {
        ...
        {                    
            ...
            else if (lang == "cpp" && mode.Equals("file"))
            {
                ...
                {
                    // output is set to cli
                    //Console.WriteLine("Size of iv = " + ivBytes.Length);
                    Console.WriteLine("char iv["+ivBytes.Length+"] = " + "{ 0x" + BitConverter.ToString(ivBytes).Replace("-", ", 0x") + " };");

                    //Console.WriteLine("Size of key = " + password.Length);
                    Console.WriteLine("\nchar key["+ password.Length + "] = " + "{ 0x" + BitConverter.ToString(Encoding.UTF8.GetBytes(password)).Replace("-", ", 0x") + " };");

                    //Console.WriteLine("Size of encrypted shellcode = " + encryptedShellcode.Length);
                    Console.WriteLine("\nunsigned char payload["+ encryptedShellcode.Length + "] = " + "{ 0x" + BitConverter.ToString(encryptedShellcode).Replace("-", ", 0x") + " };");
                }
            }
            //This will take one or more strings create a C++ template file which decrypts the strings
