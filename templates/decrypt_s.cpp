...
int Decrypt(char * payload, unsigned int payload_len, char * key, size_t keylen, char * iv) {
        ...
        if (!CryptSetKeyParam(hKey, KP_IV, reinterpret_cast<BYTE*>(iv), 0)) {
				return -1;
		}
		if (!CryptDecrypt(hKey, (HCRYPTHASH)NULL, 0, 0, reinterpret_cast<BYTE*>(payload), reinterpret_cast<DWORD*>(&payload_len))) {
				return -1;
		}
